...
4. **Sequence phases as spec'd, commit per phase** (commit-as-we-go — never trust an uncommitted working tree). Production-dark behind the gates the spec names. **Delegate the mechanical bulk** within a phase — a codemod, a migrate-N-sites sweep, a rename — to the worktree-isolated `implementer` agent while you draft the next phase or the ESLint rule; keep the judgment sites and the SSOT/rule design in the main session.
