...
**A `gateway()`-on-a-write must be provably intentional.** Every write that uses `gateway()` instead of `requireAgentWriteAccess` MUST carry an explicit allowlist entry (const or inline reason). Otherwise "intentionally lockdown-free" is indistinguishable from "forgot the gate" — the exact bug class behind the site-content hole (2026-07-02).

⚠ **THIS IS HELD BY CONVENTION, NOT BY THE CENSUS — corrected 2026-08-18.** This rule previously ended *"An ungated-for-lockdown write with no allowlist entry FAILS the census."* **That was false.** `buildActionCensus()`'s `expectedGateFamily()` special-cases only `app/(admin)/`; everywhere else ANY recognised gate satisfies it — `gateway`, `gatewaySSR`, `requireAgentWriteAccess` and `getTenantSession` are interchangeable to it. It never parses for mutation verbs, and it never reads an allowlist reason. The two files that do carry one (`lib/deposits/disburse.ts`, `lib/deposits/calculateReturn.ts`) would pass identically without it.

So the **ADDENDUM_57G subscription-lockdown gate on a money path is convention-enforced**, and this file asserted a mechanism that does not exist — the inverse stale claim, on the worst possible surface. Mechanising it is **M-003**; until that ships, treat this rule as reviewer discipline and do not cite the census as its backstop.
**UNENFORCEABLE** — MECHANISABLE (rung: check · blast: money) — twin of `CLAUDE.md`'s "requireAgentWriteAccess for ALL agent-side mutations" rule, same mechanism, not re-annotated there. This overstates Cat-15 as implemented. Read `buildActionCensus()` (`scripts/security/server-action-census.mjs`): `expectedGateFamily()` only special-cases `app/(admin)/`; every other file just needs ANY recognized gate present (`gateway`, `gatewaySSR`, `requireAgentWriteAccess`, `getTenantSession`, … are all equally acceptable). It does not parse for mutation verbs (`.update(`/`.insert(`/`.upsert(`) and does not require an allowlist entry for a write gated with bare `gateway()`. The two files that DO carry "intentional gateway()-on-write" allowlist reasons (`lib/deposits/disburse.ts`, `lib/deposits/calculateReturn.ts`) would pass the census identically without those entries — the reasons are documentation, not something the script reads to make a pass/fail decision. Sketch: scan each gated file's body for a mutation call and, if `gateway()`/`gatewaySSR()` is the only gate present, require an allowlist entry.
