...
**UNENFORCEABLE** — MECHANISABLE (rung: check · blast: other) — twin of the generalised bullet below, same mechanism, not re-annotated here in full. Nothing scans these specific legal pages for a bare-space `</strong> text` pattern and fails.
...
**UNENFORCEABLE** — MECHANISABLE (rung: check · blast: other) — sketch: a check scans `app/(public)/**` JSX for `</strong>` (or `</span>`/`</em>`) immediately followed by a bare space and text, and fails on the un-escaped form.
