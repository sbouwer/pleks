...
- Do not split a `.ts` → `.tsx` migration across commits — delete the predecessor in the SAME commit that introduces the successor. A surviving `.ts` shadow makes TypeScript resolve every unextended import to the stale file, silently masking the new one. Case-only collisions (`Card.ts` beside `card.tsx`) are reported as a separate class, because the remedy is a rename, not a delete. <!-- @enforced check:check-extension-stem-pairs -->
- The same rule for the **js-family and multi-extension spellings** — `.js`/`.jsx`/`.mjs`/`.cjs` → `.ts`, and `.mts`/`.cts` — is NOT covered by that check.
  **UNENFORCEABLE** — MECHANISABLE (rung: check · blast: other) — and it is the *more* dangerous half, which is why it gets its own line rather than a qualifier on the one above. `allowJs` is on and webpack's resolve order puts `.js`/`.mjs` AHEAD of `.ts`/`.tsx`, so a surviving `foo.js` beside a new `foo.ts` shadows it at BUNDLE time while `tsc` happily typechecks the new file — typecheck green, runtime stale. The covered `.ts`-over-`.tsx` direction is at least visible to `tsc`. Full sketch → **M-063** in `docs/MECHANISABLE.md`.
