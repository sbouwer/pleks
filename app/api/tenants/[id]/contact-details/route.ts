/**
 * app/api/tenants/[id]/contact-details/route.ts — FILL: one-line purpose
 *
...
export async function POST(req: NextRequest, { params }: RouteContext) {
  ...
  const valid = await verifyTenantOwnership(service, tenantId, contactId, membership.org_id)
  if (!valid) return NextResponse.json({ error: "Tenant not found" }, { status: 404 })
