...
| Addendum | Parent | Status | Description |
|----------|--------|--------|-------------|
| `ADDENDUM_60A_INSURANCE_CHECKLIST.md` | BUILD_60, 59 | 📝 | 13-item insurance verification checklist · 3-state model (confirmed/unknown/N/A) · auto-derived policy header · agent inline + owner magic-link (re-uses BUILD_60 info-request flow) + broker brief PDF · renewal banner with single T+7 reminder · feeds BUILD_59 critical-incident broker email · feeds BUILD_60 completeness widget. Schema amends `012_property_extensions.sql` per amend-forward rule (3 catalogue/instance/audit tables + `property_insurance_renewal_reminders`); catalogue seed amends `006_seed.sql`. No new migration file. |
