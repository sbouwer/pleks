...
| 8 | Infrastructure fixes (cross-cutting) | ADDENDUM_00A–H | 🔨 00A–E done · 00F–H spec'd |
...
| `ADDENDUM_00F_USER_FEEDBACK.md` | ✅ | **Shipped 2026-04-30** on `feat/build-00f-user-feedback` (PR #36). Migration §16 applied to live DB: `feedback_submissions` + `feedback_replies` with RLS (submitter/org-admin/platform-admin). `POST /api/feedback` resolves role+org for all 4 user types (agent, tenant, landlord, supplier); rate-limited 5/24h. `FeedbackButton` (floating) + `FeedbackDialog` mounted in dashboard, tenant, landlord, supplier layouts. Org-admin inbox at `/settings/feedback`; platform-admin inbox at `/admin/feedback`. `feedback.reply` + `feedback.daily_digest` email templates registered. Daily digest cron wired into orchestrator. |
