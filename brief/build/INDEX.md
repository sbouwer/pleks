...
> Last updated: 2026-04-19 — ADDENDUM_00D_CI_CD spec'd: first-class GitHub Actions CI, semantic-release (tag + CHANGELOG + GitHub Release, no npm publish), Dependabot with grouped weekly PRs, conventional-commits enforcement via PR-title lint, Trivy CVE scanning, security-audit CI subset (localhost-free categories 1/2/5/7). BUILD_62 (Authentication & Account Security) spec'd: claims reserved BUILD_62 slot for native Supabase hardening (Part A) + passkey/WebAuthn layer (Part B). BUILD_61 scope expanded to absorb subdomain + role namespace + route security; ADDENDUM_61B added.
...
| 8 | Infrastructure fixes (cross-cutting) | ADDENDUM_00A–D | 🔨 In progress (00D spec'd) |
...
| 60 | `BUILD_60_SMART_PROPERTY_SETUP.md` | ✅ | **All 22 phases shipped.** Scenario picker (17 SA scenarios r1–r7, c1–c6, m1–m4) with ownership mode, educational bullets, universal questions (scheme/WiFi/cell/backup power), scenario follow-up, operating hours, landlord, units, insurance, documents steps. `buildProfile()` + `buildSkeletonUnits()` pure derivation (`lib/properties/`). Completeness widget with 30-day lifetime + 7-day dismiss cooldown, per-item action buttons. Property info requests for owner/broker/self tracks — create, remind (owner T+3/T+7, broker T+5, self T+30 email), public submit form with per-topic writebacks, daily cron with optimistic-concurrency guard. React Email templates for all 8 topics × initial + reminder + completion notify + self-track nudge. Bulk classifier (`/properties/classify`) + reclassify dialog. Tier gate moved to lease activation (`canActivateLease`). Scenario label + operating hours surfaced on property OverviewTab. Mobile-responsive wizard shell. Open: ADDENDUM_60A (insurance checklist). |
| 61 | `BUILD_61_ROUTE_ALIGNMENT.md` | 📝 | **Expanded scope.** (A) Original route-naming renames: `/payments` → `/billing`, `/contractors` → `/suppliers`, `/settings/finance` → `/settings/deposits`, `/settings/billing` → `/settings/subscription`, `/settings/communication/templates` → `/settings/documents/templates`, profile/details split with `/settings/profile` repurposed as user-profile stub. Matching `/api/*` renames. Orphan audit. (B) Role namespace renames: `(portal)` route group → `(tenant)` with URL `/portal/*` → `/tenant/*`; `(landlord-portal)` → `(landlord)` with URL unchanged; `(contractor)` → `(supplier)` with URL `/contractor/*` → `/supplier/*`. Schema names retained (`portal_view`, `tenant_portal_login`, `portal.tenant_invite`, `pleks_portal` — these document concepts not URLs). (C) Subdomain: `app.pleks.co.za` for the product, apex `pleks.co.za` for marketing; central `/login` gateway; hostname-resolution helper with dev/preview short-circuits. (D) Security: canonical route manifest at `lib/routing/manifest.ts`, middleware reads from manifest for role enforcement; three-layer model (middleware coarse gate + server-component auth helpers + RLS data gate). 15 permanent HTTP 308 redirects. `NEXT_PUBLIC_APP_URL` as single source of truth for outbound URL generation. Cookies scoped to `app.pleks.co.za` only. Localhost unchanged (single dev server at :3000 defaults to 'app' context). **Does not touch**: webhooks, cron routes, `/apply/*`, `/admin/*`, `/demo/*`, `lib/portal/*` directory (feature-domain naming), database table names. User-profile buildout deferred to BUILD_62. Multi-role navigation shipped separately as ADDENDUM_61B. Estimated ~200–300 file touches. Ships before BUILD_63. |
...
| `ADDENDUM_61B_MULTI_ROLE_NAVIGATION.md` | BUILD_61 | 📝 | Multi-role user handling. Role-membership resolution at login from `user_orgs` / `user_orgs_tenants` / `user_orgs_landlords` / `user_orgs_contractors` bridge tables. Single-role users auto-route to workspace; multi-role users land at `/` role-selector; zero-role users redirect to `/onboarding`. In-session `RoleSwitcher` component visible in top-left of every role workspace layout when `available.length > 1`. New cookies: `pleks_active_role` (7 days) + `pleks_available_roles` (5 min cache). `/switch-role` POST endpoint with server-side membership validation + audit_log entry. `/403` page with switcher affordance. `ROLE_DEFAULT_ROUTES` manifest. RLS unchanged — cookie is routing state only, not data-access claim (tampered cookie → wrong URL → middleware 403, not data breach). Edge cases spec'd: role-switcher redirects to role default (not current-page's role-equivalent), multi-org agent case separate from multi-role, membership revocation invalidates cookie within 5 min, delivery-alert deep link flow (BUILD_63 §9.5) sets active role from URL redirect parameter after auth. |
...
| `ADDENDUM_00D_CI_CD.md` | 📝 | **First-class CI/CD and version-control automation.** Three GitHub Actions workflows: `ci.yml` (lint+typecheck / security:ci subset / Trivy CVE / pr-title — NO build job, D-CI-05: Vercel's preview deploy handles build validation), `release.yml` (main-only, gated on CI, runs semantic-release), `pr-title.yml` (re-validates title on edit). Semantic-release config: conventional-commits preset, tag format `v${version}`, first tag `v1.0.0`, plugins = commit-analyzer + release-notes-generator + github only (D-CI-09: NOT changelog, NOT git, NOT npm — release workflow never pushes back to `main`, sidesteps GitHub's non-selectable-bot-bypass papercut; GitHub Releases page is the changelog). Dependabot weekly npm (grouped by ecosystem: react, supabase, tiptap, fullcalendar, tanstack, semantic-release; production vs dev split) + monthly github-actions. `.nvmrc` pins Node 20; `package.json engines.node >=20.9.0`. New `security:ci` script runs localhost-free subset (cats 1, 2, 5, 7 = Supabase-direct RLS/isolation checks) with graceful skip if `CI_SUPABASE_URL`/`CI_SUPABASE_SERVICE_ROLE_KEY` absent. PR-title lint via `amannn/action-semantic-pull-request`. PR template checklist covers local `npm run check`, security touch-points (RLS/auth/trust/POPIA/webhooks/consent), mobile + preview-deploy verification. Squash-merge-only enforced; branch protection required status checks: `Lint & Typecheck`, `Security (static Supabase checks)`, `Dependency CVE scan (Trivy)`, `PR title (Conventional Commits)` — build validation handled by Vercel's preview deploy (not a GitHub check, but visible on the PR). Bypass list: Repository admin only (emergency hotfix). Release workflow does not push to `main` so no bot bypass needed. 20 design decisions logged (D-CI-01 through D-CI-20). Tier 2 follow-on: test-matrix + coverage when vitest lands. Tier 3 follow-on: CODEOWNERS + required approvals when team grows. Standing CC instruction appended to `CLAUDE.md` covering conventional-commit format. Rollback documented: delete `.github/` + lift branch protection, 5-minute job. Depends on zero other builds. Vercel continues to handle deployment unchanged — CI gates merges, Vercel gates deploys. **Blocker for CC handoff:** §4.6 GitHub UI toggles (squash-only merge, ruleset on `main` with required status checks, Repository-admin bypass, `production` environment, optional CI secrets) must be set by Stéan — CC cannot do repo-settings work. |
...
| `013_auth_security.sql` | Authentication security | BUILD_62 Part A — `auth_events` (append-only, 7-year retention, 24-event-type CHECK enum), `device_fingerprints` (soft-delete only), `login_notifications_sent` (unique user×device dedup), `step_up_challenges` (5-min ephemeral, single-use), `is_mfa_fresh(int)` helper, `purge_old_auth_events()` cron target. BUILD_63 `tenant_portal_login` events consumed here rather than creating a parallel table. |
| `014_passkeys.sql` | Passkeys (WebAuthn) | BUILD_62 Part B — `user_passkeys` (credential_id unique, public_key, counter, transports, device_type for singleDevice/multiDevice, AAGUID, rp_id, origin, soft-delete via revoked_at), `passkey_challenges` (ephemeral, 5-min TTL, service-only access). `pg_cron` cleanup of expired challenges every 15 min. |
