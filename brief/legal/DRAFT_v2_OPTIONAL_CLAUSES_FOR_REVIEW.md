# DRAFT v2: Optional Lease Clauses — Legal Feedback Applied

**Status:** REVISED DRAFT — incorporates preliminary legal review feedback. Requires final attorney sign-off before seeding.

**Changes from v1:**
- 🔴 Parking indemnity: gross negligence carve-out added
- ⚠️ Pet deposit: restructured as allocated portion of total deposit, not separate deposit
- ⚠️ Pet consent withdrawal: changed to "not less than 14 days"
- ⚠️ All indemnities: added "subject to applicable law" / "excluding gross negligence or wilful misconduct" qualifiers
- ⚠️ Utilities no-reduction clause: softened with "unless otherwise required by law"
- ⚠️ Borehole: explicit Schedule 1 domestic use exemption added
- ⚠️ Solar feed-in: added "subject to applicable municipal approval requirements"
- 🟢 Annexure D hierarchy rule: added globally to general clause recommendations
- 🟢 Breed restrictions: confirmed generic wording is correct — no change
...
**Changes from v1:**
- Pet deposit restructured as an allocated portion of the total deposit under `{{ref:rental_deposit}}`, not a separate deposit — avoids Tribunal disputes about multiple deposits under RHA
- Indemnity qualified with "subject to applicable law" and "excluding gross negligence or wilful misconduct"
- Consent withdrawal changed to "not less than fourteen days" — gives flexibility
- Breed restriction reference kept generic (no hardcoded lists — correct per legal advice)
...
**Changes from v1:**
- 🔴 **Critical fix:** Indemnity in sub-clause 6 now explicitly carves out gross negligence and wilful misconduct. The original "howsoever caused, whether by the negligence of the lessor" was unenforceable under CPA s48 for natural person lessees. New wording: "save where any loss or damage arises from the gross negligence or wilful misconduct of the lessor" — this preserves the general indemnity while complying with CPA.
...
**Changes from v1:**
- Sub-clause 7 (solar feed-in): added "and subject to applicable municipal approval requirements" — acknowledges varying municipal SSEG programmes
- Sub-clause 8 (borehole): added "save for reasonable domestic use as permitted under Schedule 1 of the National Water Act" — correctly distinguishes exempt domestic use from licensed commercial/irrigation use
- Sub-clause 9 (no rental reduction): softened from absolute prohibition to "shall not in itself constitute grounds... unless otherwise required by law" — CPA-safe
...
**Changes from v1:**
- Sub-clause 3: added "save where such failure arises from the gross negligence or wilful misconduct of the lessor" — consistent CPA carve-out pattern across all clauses

---

## 5. Global recommendation: Annexure D hierarchy rule

**Action:** Add the following to the `general` clause (existing required clause) or as a standalone addendum instruction in the lease generation system:

```
In the event of any conflict between the provisions of Annexure D: Special Agreements and the main body of this agreement, the provisions of Annexure D shall prevail to the extent of such conflict.
```

**Implementation note:** This should be appended to the existing `general` clause body template or added as a new required sub-clause. Since `general` is a required clause that always appears, this hierarchy rule will always be present. This prevents contradictions in generated leases where Annexure D pet permissions, parking specifics, or utility arrangements conflict with main body defaults.

**Do NOT create a new optional clause for this** — it must always be present. Options:
1. Amend the `general` clause body template (preferred — single source of truth)
2. Add a new required clause `annexure_hierarchy` at sort_order 3350 (after `general` at 3300)

---

## 6. Seed migration template (updated)

Once final attorney sign-off received, create `supabase/migrations/011_optional_clauses_seed.sql`:

```sql
-- 011_optional_clauses_seed.sql
-- Additional optional clauses for unit feature auto-mapping.
-- ATTORNEY REVIEWED: [date] by [attorney name]
-- REVISION: v2 — incorporates preliminary legal review feedback
...
-- Annexure D hierarchy rule — amend the general clause
...
## 7. Remaining review items for final sign-off

| Item | Status | Action needed |
|------|--------|---------------|
| Pet deposit as allocated portion | ✅ Restructured | Attorney to confirm wording |
| Parking gross negligence carve-out | ✅ Fixed | Attorney to confirm CPA compliance |
| Borehole Schedule 1 exemption | ✅ Added | Attorney to confirm accuracy |
| Solar municipal approval | ✅ Added | Attorney to confirm sufficiency |
| No-rental-reduction softening | ✅ Fixed | Attorney to confirm wording |
| All indemnities CPA-qualified | ✅ Fixed | Attorney to confirm consistency |
| Annexure D hierarchy rule | ✅ Drafted | Attorney to confirm placement (general clause vs standalone) |
| Breed restrictions | ✅ No change | Confirmed generic wording is correct |
| Films & Publications Act citation | ✅ No change | Act 65 of 1996 still valid |
| Electronic Communications Act | ✅ No change | Act 36 of 2005 confirmed |
| National Road Traffic Act | ✅ No change | Act 93 of 1996 confirmed |
