...
## Purpose B9 — Application fee processing (payment gateway)

- **Purpose name (internal):** `application_fee_processing`
- **Description:** Accept the applicant's rental application fee directly via the payment gateway (see Appendix A6). The fee is a Pleks-to-applicant service charge structured to cover (a) the cost of the underlying credit bureau report payable to the credit bureau aggregator (see Purpose B4) and (b) Pleks's cost of operating and maintaining the application-processing service. **The agency receives no portion of this fee; funds from the application fee do not flow to the agency under any tier or commercial arrangement between Pleks and the agency.** The fee amount is set by Pleks from time to time based on changes in the underlying credit-bureau-aggregator pricing (see Purpose B4) and Pleks operating costs, and is disclosed to the applicant at the point of payment before the transaction is committed; variations in the fee amount are reflected in the in-product disclosure, not by amending this register.
- **Controller:** Pleks (Responsible Party — the transaction is between Pleks and the applicant, not between agency and applicant at the payment gateway layer); agency is notified of the payment but does not originate it
...
- **Personal data categories:** applicant name, email, payment amount, transaction reference, card or EFT method indicator (not full card number — the payment gateway is the PCI boundary; SA ID number is not passed to the payment gateway)
- **Data subject categories:** rental applicants
- **Recipients / Operators:** the payment gateway (SA-domiciled — see Appendix A6)
- **Cross-border transfer:** No — the payment gateway is SA-domiciled. Basis: domestic processing.
- **Retention:** 5 years (Tax Administration Act)
- **DPIA required:** No — standard consumer payment processing; Pleks is on the PCI boundary via the payment gateway (not a direct card-handler)
- **Related specifications:** BUILD_16 (application pipeline)
...
- **Recipients / Operators:** Supabase (storage), Resend (resolution emails), AI model provider (export narrative — Purpose B22 sub-purpose `popia_export_narrative`)
- **Cross-border transfer:** Yes — see Appendix B.
...
- **Recipients / Operators:** Supabase (storage of documents and structured CDD records); the AI model provider (only if AI-assisted OCR or document extraction is used via `lib/ai/client.ts` — covered under Purpose B22)
...
- **Recipients / Operators (when active):** Supabase, Resend (email), Africa's Talking (WhatsApp and SMS), possibly the AI model provider (if AI-generated marketing content is used — Purpose B22)
