...
export function LogMaintenanceForm({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (tenant) formData.set("tenant_id", tenant.id)
    ...
    if (triageConfirmed && overrideCategory) {
      formData.set("category_override", overrideCategory)
      formData.set("urgency_override", overrideUrgency || "routine")
    }
  ...
  const filteredContractors = effectiveCategory
    ? contractors.filter((c) => {
        const keywords = CATEGORY_SPECIALITY[effectiveCategory] ?? []
        if (keywords.length === 0) return true
        return c.specialities.some((s) => keywords.some((k) => s.toLowerCase().includes(k)))
      })
    : contractors

  const pickableContractors = filteredContractors.length > 0 ? filteredContractors : contractors
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              {tenant ? (
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Loading tenant…</p>
              ) : (
                <p className="text-xs text-muted-foreground mt-0.5">No active tenant</p>
              )}
            </div>
            <button
              type="button"
              onClick={() => { setUnitId(""); setPropertyId(""); }}
      ...
      {/* ─── AI classification ──────────────────────────── */}
      {(triaging || triageResult) && !triageConfirmed && (
          ...
          {triageResult && !triaging && (
            <>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      placeholder="e.g. Tenant has small children, upgrading urgency"
                      className="text-sm"
                    />
                  </div>
                </div>
              ...
              <div className="flex gap-2 flex-wrap">
                {!overriding ? (
                  <>
                    <Button type="button" size="sm" className="h-7 text-xs" onClick={handleConfirmTriage}>
                      <CheckCircle2 className="h-3.5 w-3.5 mr-1" />
                      Confirm
                    </Button>
                    <Button type="button" size="sm" variant="ghost" className="h-7 text-xs" onClick={() => { setOverriding(true); setOverrideCategory(triageResult.category); setOverrideUrgency(triageResult.urgency) }}>
                  ...
                  <>
                    <Button type="button" size="sm" className="h-7 text-xs" onClick={handleConfirmOverride}>
                      <CheckCircle2 className="h-3.5 w-3.5 mr-1" />
                      Confirm override
                    </Button>
                    <Button type="button" size="sm" variant="ghost" className="h-7 text-xs" onClick={() => setOverriding(false)}>
                      Cancel
                    </Button>
                  </>
                )}
      ...
      {triageConfirmed && (
        <div className="rounded-xl border border-border/60 bg-surface-elevated px-5 py-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button type="button" onClick={handleResetTriage} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1">
              <RotateCcw className="h-3 w-3" /> Retriage
            </button>
          </div>
          ...
          {loadingPreferred ? (
            <p className="text-xs text-muted-foreground">Finding preferred contractor…</p>
          ) : preferredContractor && !showContractorPicker ? (
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-medium">{preferredContractor.name}</p>
                <p className="text-xs text-muted-foreground">
                  Preferred for {CATEGORY_LABELS[effectiveCategory] ?? effectiveCategory}{preferredContractor.scope === "property" ? " at this property" : ""}
                  {preferredContractor.phone ? ` · ${preferredContractor.phone}` : ""}
                </p>
              </div>
              <button type="button" onClick={() => setShowContractorPicker(true)} className="text-xs text-brand hover:underline shrink-0">
                Change
              </button>
            </div>
          ) : (
          ...
          <div className="space-y-1.5">
            ...
            {estimatedCostCents > 0 && (
              <p className={`text-xs ${aboveThreshold ? "text-warning" : "text-success"}`}>
                {aboveThreshold
                  ? `⚠️ Above approval limit (${formatZAR(approvalThresholdCents)}) — landlord will be notified for approval`
                  : `✅ Within approval limit (${formatZAR(approvalThresholdCents)})`}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border/60 bg-surface-elevated px-5 py-4 space-y-3">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            rows={2}
            placeholder="e.g. Key at reception. Tenant works from home — call 30 min before. No power cuts before 10am."
        ...
        <div className="space-y-1.5">
          ...
          <Textarea
            ...
            rows={2}
            placeholder="e.g. Noise restrictions after 17:00"
            className="resize-none text-sm"
          />
        </div>
      </div>
