...
/**
 * components/properties/VacantSection.tsx — Vacant unit panel for selecting prospective tenants and initiating lease creation
 *
 * Auth:   gateway (dashboard layout)
 * Data:   setProspectiveTenants server action; TenantPicker for tenant search
 */
...
export function VacantSection({
  ...
  const [primary, setPrimary] = useState<PickedTenant | null>(
    initialTenantId && initialTenantName ? { id: initialTenantId, name: initialTenantName, contact_id: null, phone: null, entity_type: null, juristic_type: null, turnover_under_2m: null, asset_value_under_2m: null, size_bands_captured_at: null } : null
  )
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="rounded-xl border border-border/60 bg-surface-elevated px-5 py-4 flex flex-col gap-3">
        ...
        {primary ? (
          <div className="flex flex-col gap-2">
            ...
            {addingCo ? (
              <div className="space-y-1">
                ...
                <TenantPicker orgId={orgId} onSelect={handleSelectCo}
                  trigger={
          ...
          <>
            ...
            <TenantPicker orgId={orgId} onSelect={handleSelectPrimary}
              trigger={
                <button type="button" className="w-full flex items-center gap-3 rounded-lg border border-border/60 bg-background px-3 py-2 text-left hover:bg-muted/30 transition-colors">
                  <UserRound className="size-4 text-muted-foreground flex-shrink-0" />
      ...
      <div className="rounded-xl border border-border/60 bg-surface-elevated px-5 py-4 flex flex-col gap-3">
        ...
        <InlineLink href={leaseHref} withArrow>Create a lease</InlineLink>
      </div>
    </div>
  )
}
