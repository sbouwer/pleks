...
### M-096 — `context-budget.js` bills one API response once per content block, and it is the instrument every session reads
...
- **Related:** M-094 (a defect two artefacts inherit together agrees with itself — same class, different subject) · E16 (where this was found, and whose corroboration claim it retracted)
