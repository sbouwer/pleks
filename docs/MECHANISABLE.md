...
- **Note the interaction with M-108, and DO NOT FIX HALF OF IT.** `declareDirectors` is the sole writer of BOTH `applications.entity_type`'s organisation value and `application_co_applicants.is_surety_director` — the latter is read in six places and written in exactly one, this one. So today: nothing writes `entity_type`, the juristic branch in `app/api/billing/screening/route.ts` never fires, and the surety gate is **silently open**. Wire `entity_type` alone and `suretyCount` still counts `is_surety_director = true`, which nothing writes, so it is 0 and `validateJuristicParties` **blocks every juristic application at payment** — silently closed. The two failure modes are opposite and the halves must land together. Sequencing this wrong takes a dormant flow and makes it a customer-facing outage.
