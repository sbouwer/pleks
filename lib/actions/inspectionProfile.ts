...
/**
 * lib/actions/inspectionProfile.ts — generates a unit inspection profile from its type/features (no physical inspection)
 *
 * Auth:   requireAgentWriteAccess("edit_property")
 * Data:   reads units; upserts unit_inspection_profiles; replaces unit_inspection_profile_rooms
 * Notes:  idempotent — safe to re-run; upserts the header and rebuilds rooms each call
 */
