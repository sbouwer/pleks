...
/**
 * lib/actions/propertyInfoRequests.ts — property-info-request server actions (create, send reminder, dismiss)
 *
 * Auth:   requireAgentWriteAccess; create enforces gw.orgId === params.orgId, and reminder/dismiss
 *         org-scope the request read + update (.eq("org_id", orgId)).
 * Data:   property_info_requests via the gateway service client.
 */
