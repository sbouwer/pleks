...
/**
 * STEP 1 — Consume outstanding interest charges (oldest first).
 * Returns { interestApplied, remaining }.
 */
async function applyInterestCharges(
  ...
  for (const charge of charges) {
    ...
    await supabase
      .from("arrears_interest_charges")
      .update({ waived: true, waived_reason: `Consumed by payment ${paymentId}`, waived_at: new Date().toISOString() })
      .eq("id", charge.id)
    remaining -= apply
  ...
  if (interestApplied > 0) {
    await supabase.from("payments").update({ interest_applied_cents: interestApplied }).eq("id", paymentId)
  ...
  return { interestApplied, remaining }
}

/**
 * STEP 2 — Apply remainder to open rent invoices (oldest first).
 * Returns { rentApplied, remaining }.
 */
async function applyRentInvoices(
  supabase: SupabaseServiceClient,
  leaseId: string,
  remaining: number
...
export async function allocatePayment(
  ...
  return {
    ...
    rentAppliedCents: step2.rentApplied,
    surplusCents: step2.remaining,
  }
}
