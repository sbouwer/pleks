/**
 * lib/inspections/profileHelpers.ts — persists a completed pre-listing inspection's rooms as the unit's permanent inspection profile
 *
 * Data:   inspection_rooms (read); unit_inspection_profiles, unit_inspection_profile_rooms (upsert/replace)
 * Notes:  Idempotent — upserts the per-unit profile header then replaces all its room rows. Caller supplies the org-scoped db.
 */
