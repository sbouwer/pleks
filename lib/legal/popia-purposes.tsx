/**
 * lib/legal/popia-purposes.tsx — POPIA s17/s18 processing-purpose definitions
 *
 * Notes:  Single source of truth for all 38 processing purposes.
 *         12 Part A (Pleks as Responsible Party) + 26 Part B (Pleks as Operator).
 *         B26 (criminal-background screening) was REMOVED 2026-08-14: the capability was cancelled
 *         per ADDENDUM_14E (2026-05-21, Huru + Estate bundle), is refused by lib/screening/recordDecision.ts,
 *         and its table carries RLS WITH CHECK false — yet the public register still declared it with a
 *         "Completed" DPIA. Do not reinstate without a live implementation AND counsel sign-off on
 *         POPIA s33 / s57. Ids are NOT renumbered — B27 keeps its id so external references stay valid.
...
export interface PopiaProcessingPurpose {
  ...
  dpia?: string           // from pm-row DPIA (B27 only)
...
export const POPIA_PURPOSES: readonly PopiaProcessingPurpose[] = [
  ...
  },
  {
    ...
    recipients: "credit bureau aggregator — SA-domiciled, domestic processing (see §C); underlying bureaus via credit bureau aggregator; database and storage provider; AI model provider (FitScore generation — B5)",
