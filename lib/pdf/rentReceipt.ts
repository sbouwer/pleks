/**
 * lib/pdf/rentReceipt.ts — builds the branded HTML for a rent-payment receipt PDF
 *
 * Notes:  Pure HTML/CSS string builder from a ReceiptData struct — no DB, no PDF engine (caller renders to PDF).
...
function buildCSS(fontStack: string, accent: string): string {
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  hr { border: none; border-top: 2px solid ${accent}; margin: 8px 0 20px; }
  ...
  .amount-box { background: #f8fafc; border: 1px solid #e2e8f0; border-top: 3px solid ${accent}; border-radius: 6px; padding: 12px 16px; margin-bottom: 16px; }
...
export function buildRentReceiptHTML(data: ReceiptData): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8">${fontLink}<style>${css}</style></head><body>
    ...
    <dl class="info-grid">
      ...
      <dt>Payment method</dt><dd>${methodLabel}</dd>
      ${refRow}
    </dl>
