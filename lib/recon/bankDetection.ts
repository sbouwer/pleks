/**
 * lib/recon/bankDetection.ts — detects the SA bank from statement text and supplies per-bank + system extraction prompts for the AI parser
 *
 * Notes:  Pure logic + prompt constants. Per-bank hints encode each bank's statement quirks (date/amount formats,
 *         two-line descriptions, signed vs. split debit/credit columns) to steer the LLM transaction extraction.
 */
