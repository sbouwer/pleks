...
function buildAgentContact(officeHours: string, emergencyLine: string, instructions: string | null, accent: string): string {
  if (!officeHours && !emergencyLine) return ""
  const hoursRow = officeHours ? `<tr><td style="width:160px;color:#64748b;">Office hours</td><td>${officeHours}</td></tr>` : ""
  const emergencyRow = emergencyLine ? `<tr><td style="color:#64748b;">Emergency (after hours)</td><td>${emergencyLine}</td></tr>` : ""
  const instrRow = instructions ? `<tr><td style="color:#64748b;vertical-align:top;">Instructions</td><td>${instructions}</td></tr>` : ""
  return `<h2 style="color:${accent};">Agent Contact</h2><table>${hoursRow}${emergencyRow}${instrRow}</table>`
}
...
function buildPage1(data: WelcomePackData, org: ReportBranding): string {
  ...
  return `
    ...
    ${buildAgentContact(officeHours, emergencyLine, org.hours.emergencyInstructions, accent)}
  `
}
...
export function buildWelcomePackHTML(
  ...
  const body = `
    ...
    ${hasProperties ? buildPage2(data) : ""}
    ${hasProperties ? buildPage3(data) : ""}
    ${hasProperties ? buildPage4(data) : ""}
    ${hasProperties ? buildPage5(data) : ""}
    ${hasProperties ? buildPage6(recs, accent) : ""}
    ${footer}
  `
