/**
 * lib/reports/tenantWelcomePackHTML.ts — HTML renderer for the tenant welcome pack report
 *
 * Data:  receives TenantWelcomePackData + ReportBranding; pure HTML string output
 * Notes: inlined CSS, multi-page layout; org.org_name used in footer attribution
 */
...
export function buildTenantWelcomePackHTML(
  ...
  const dateStr = fmtDateLongZA(data.generatedAt)
  const footer = `<div class="footer">Generated by ${org.org_name} via Pleks · pleks.co.za — ${dateStr}</div>`
  const toolbarHtml = toolbar ? buildToolbar(toolbar, accent) : ""
  const closingHtml = signatureAttribution
    ? `<p style="font-size:10px;color:#64748b;margin-top:24px;text-align:right;font-style:italic;">${signatureAttribution}</p>`
    : ""
