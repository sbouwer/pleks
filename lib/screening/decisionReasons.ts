 ...
 * Notes:  ⚠ CD-DRAFT VALUES — pending retained-counsel review (same sign-off block as Q1(b-lite)/Q2(a)).
 *         The schema SHAPE is final (SPEC §2.2); the VALUES below may be revised by counsel (SPEC §9
 *         flags six items). DO NOT apply the generated migration to prod until counsel ticks the values —
 *         a placeholder code that counsel later renames would leave prod rows tagged with a dead code.
 *         decline_reason_text (the agent-discretion free-text sibling, SPEC §3/§9-item-1) is NOT added here
 *         — it is the most counsel-contested item; deferred until item 1 is ruled.
 */
...
export const ADVERSE_FACTOR_CODES = [
  ...
  "adverse_landlord_reference_unreachable",
  "adverse_employer_reference_negative",
  "adverse_employer_reference_unreachable",
  // rental history
