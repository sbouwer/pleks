...
export async function runFitScoreOrchestrator(
  ...
  const { data: app, error: appErr } = await supabase
    .from('applications')
    .select(`
      id, org_id, listing_id, is_foreign_national, gross_monthly_income_cents,
      bank_statement_extracted, searchworx_extracted_data, income_evidence_tier,
      identity_match_status, employer_verification_status, salary_reconciliation_status,
      document_consistency_status, bank_account_ownership_status,
      pleks_network_history_status, pleks_network_tenancy_count, fitscore_inputs_hash
    `)
    .eq('id', applicationId)
    .single()
  ...
  const primaryInput: ApplicantInput = {
    ...
    nationalityType: app.is_foreign_national ? 'foreign_national' : 'sa_citizen',
    tier1IncomeCents: null,
    tier2IncomeCents: extractBankIncome(app.bank_statement_extracted),
  ...
  const coInputs: ApplicantInput[] = (coApplicants ?? []).map((co, idx) => ({
    ...
    nationalityType: nationalityFromIdType(co.id_type),
    tier1IncomeCents: null,
    tier2IncomeCents: extractBankIncome(co.bank_statement_extracted),
