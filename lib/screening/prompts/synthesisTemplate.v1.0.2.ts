/**
 * lib/screening/prompts/synthesisTemplate.v1.0.2.ts — Deterministic synthesis paragraph for FitScore reports
 *
 ...
 *         Diff vs v1.0.1: LDP branch produces "Limited Data Profile — composite not positioned.
 *         {N} of 4 dimensions had scoreable evidence available." using countPopulatedDimensions.
 *         All dimension comparisons are null-safe (supports number | null dimension scores).
 *         Spec: ADDENDUM_14H_FITSCORE_DELIVERY.md §E.5.
 */
...
export const SYNTHESIS_TEMPLATE_VERSION = 'synthesis.v1.0.2'
...
export function buildSynthesis(data: FitScoreReportData): string {
  // Limited Data Profile (LDP) is not a low-confidence comparative score. It represents
  // insufficient evidence to support standard comparative placement.
  // Do NOT collapse 'insufficient' to 'low' during any refactor.
  if (data.isLdp) {
    const n = countPopulatedDimensions(data)
    return [
      'Limited Data Profile — composite not positioned.',
      `${n} of 4 dimensions had scoreable evidence available.`,
