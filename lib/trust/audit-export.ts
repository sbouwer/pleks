...
async function buildXlsx(d: TrustAuditData): Promise<Buffer> {
  ...
  for (const txn of d.transactions) {
    ...
    ws1.addRow({
      date: txn.date ? new Date(txn.date).toLocaleDateString("en-ZA") : "",
  ...
  for (const dep of d.depositsHeld) {
    ws2.addRow({
      ...
      leaseStart: dep.leaseStart ? new Date(dep.leaseStart).toLocaleDateString("en-ZA") : "",
  ...
  addKV("Signed off at", new Date(d.signedOffAt).toLocaleString("en-ZA"))
  addKV("IP address", d.signedOffIp ?? "—")
  ws3.addRow([])
  addKV("Manifest hash (SHA-256)", d.manifestHash)
  addKV("Generated at", new Date(d.generatedAt).toLocaleString("en-ZA"))
