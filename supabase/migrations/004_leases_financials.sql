...
-- ═══════════════════════════════════════════════════════════════════════════════
-- §  AUDIT_IMPORT deposits (2026-07-12): "imported, rate not set"
-- ═══════════════════════════════════════════════════════════════════════════════
-- A migrating agency ALREADY HOLDS its tenants' deposits. The importer recorded the amount on the
-- lease and posted nothing to the deposit/trust ledger, so RHA s5(3) interest never accrued and a
-- move-out reconciliation had no principal.
--
-- Accruing interest, though, needs a RATE — and the rate for a deposit taken years ago by another
-- system is something Pleks cannot infer. Today `accrueDepositInterest` falls back to a HARD-CODED
-- 5% p.a. when neither a deposit_interest_config nor a per-lease rate exists. Inventing a rate on
-- somebody else's money held in trust is the wrong kind of wrong: an under-accrual short-changes
-- the tenant, an over-accrual short-changes the landlord, and neither is discoverable.
--
-- So "we do not know the rate" becomes a STATE the accrual engine must respect, rather than a NULL
-- that silently means 5%:
--   'set'                — normal. The rate comes from a deposit_interest_config, or from the lease.
--   'imported_not_set'   — migrated with a deposit but no rate anywhere. Interest accrual is HELD
--                          (never accrued at a guessed rate) until an agent sets it. Ruling (Stéan
--                          2026-07-12): take the rate from the import file if it carries one, else the
--                          agency's configured rate, else hold for the agent's first touch.
-- Existing rows default to 'set' — behaviour is unchanged for every lease created through the wizard.
ALTER TABLE leases ADD COLUMN IF NOT EXISTS deposit_rate_status text NOT NULL DEFAULT 'set';
ALTER TABLE leases DROP CONSTRAINT IF EXISTS leases_deposit_rate_status_check;
ALTER TABLE leases ADD CONSTRAINT leases_deposit_rate_status_check
  CHECK (deposit_rate_status IN ('set', 'imported_not_set'));

COMMENT ON COLUMN leases.deposit_rate_status IS
  'imported_not_set = migrated deposit with no known interest rate; accrual is HELD until an agent sets one. Never accrue at a guessed rate on trust money.';
