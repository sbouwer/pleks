...
-- Idempotent — ON CONFLICT (code) DO UPDATE so re-runs refresh the catalogue.
-- ─────────────────────────────────────────────────────────────────────────────
...
VALUES
  ...
  ('POLICY_HEADER',
   ...
   'These six fields make up the basic shape of the policy. Filled in via the Insurance & risk tab — auto-confirmed here when all six are complete.',
   ARRAY['*']::text[],
   '{}'::jsonb,
   'critical', 10, true,
  ...
  ('SUBSIDENCE',
   ...
   'Critical for clay-soil regions — much of Joburg, parts of Pretoria, parts of Cape Town. Often a separate rider on the policy. Absence is a silent gap most landlords discover only at claim stage.',
   ARRAY['*']::text[],
   '{}'::jsonb,
   'critical', 50, false,
   '{}'::jsonb,
  ...
  ('PUBLIC_LIABILITY',
   ...
   'R 5–10m limits are typical for residential properties; commercial properties generally require higher limits. Verify the limit is appropriate for the property type and occupancy.',
   ARRAY['*']::text[],
   '{}'::jsonb,
   'important', 60, false,
   '{}'::jsonb,
  ...
  ('LOSS_OF_RENT',
   ...
   'Residential: "loss of rent." Commercial: "business interruption." Confirm the cover period (number of months) is realistic for the property type — a geyser burst may take 3 months; a fire, 12.',
   ARRAY['*']::text[],
   '{}'::jsonb,
   'important', 70, false,
   '{}'::jsonb,
